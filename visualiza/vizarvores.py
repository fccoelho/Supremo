...
def decisoes_por_classe():
    ...
    for d in decs:
        if not d[0]: continue
        ramo = d[1] if d[1] else 'outros'
        arvores[d[0]].add_ramo(ramo,'tronco', (0.7,0.3,0.05), 0.6, 0.6, pi/3.0)
    ...
    for d in decs:
        ...
        if d[2] in classes_min[d[0]][ramo1]:
            ...
            r2 = arvores[d[0]].add_ramo(d[2],ramo1,(0.7,0.3,0.05), 0.6, 0.5, pi/3.0)
            [r2.add_folha() for i in range(d[3]%100)]
